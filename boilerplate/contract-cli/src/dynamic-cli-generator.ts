...
export class DynamicCLIGenerator {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.info(`Witnesses: ${this.contractAnalysis.witnesses}`)
    } catch (error) {
      ...
      try {
        ...
        if (contractState) {
          ...
          if (contractNames.length > 0) {
            ...
            this.logger.info('ooo:' + JSON.stringify(api.getCounterLedgerState));
            this.logger.info('***:' + JSON.stringify(ledgerState));
            this.logger.info('>>>:' + JSON.stringify(this.contractAnalysis.ledgerState));
            // Display each ledger state variable
            for (const [stateName, stateType] of Object.entries(this.contractAnalysis.ledgerState)) {
              try {
                let value: any;

                if (stateName === 'balances' && typeof ledgerState[stateName] === 'object') {
